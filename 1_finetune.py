...
# TensorBoard
writer = SummaryWriter(log_dir='runs/caltech101_finetune')
...
for epoch in range(30):
    train(model, train_loader, optimizer, criterion, epoch, writer, "finetune")
    acc = evaluate(model, val_loader, criterion, epoch, writer, "finetune")
    print(f"[Epoch {epoch}] Accuracy: {acc:.4f}")

# Optional: save the model
os.makedirs("models", exist_ok=True)
torch.save(model.state_dict(), "models/resnet18_caltech101_finetuned.pth")
